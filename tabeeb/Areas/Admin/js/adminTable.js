...
function loadData(pageNumber) {
    if (controllerName == "doctor") return;
            ...
            if (window.FormData !== undefined) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if(result)
                        {
                            $('.profile .adminImage img').attr('src', "/Areas/Admin/images/" + filename);
                            $('.dropdown .profileImage').attr('src', "/Areas/Admin/images/" + filename);
                        }
                        else {
...
function getArabicMessageForJtable()
{
    //Localization
        var arabicMessages = {
        serverCommunicationError: 'حدث خطأ اثناء الاتصال بالسيرفر',
        loadingMessage: 'تحميل البيانات...',
        noDataAvailable: 'لا يوجد بيانات متاحة!',
        addNewRecord: 'اضافة حقل جديد',
        editRecord: 'تعديل الحقل',
        areYouSure: 'هل انت متأكد؟',
        deleteConfirmation: 'هذا الحقل سوف يتم حذفه هل انت متأكد',
        save: 'احفظ',
        saving: 'حفظ',
        cancel: 'الغاء',
        deleteText: 'اخذف',
        ...
        error: 'خطأ',
        close: 'اغلاق',
        cannotLoadOptionsFor: 'لايمكن تحميل هذه الخيارات لهذا الخقل {0}',
        pagingInfo: 'عرض {0}-{1} من {2}',
        pageSizeChangeLabel: 'عدد الحقول',
        gotoPageLabel: 'اذهب الى صفحة',
        canNotDeletedRecords: 'لا يمكن حذف {0} of {1} حقول!',
        deleteProggress: 'تم حذف {0} من {1} حقول, معالجة...'
    }
