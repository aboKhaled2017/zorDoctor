...
//return the current media[mobile|tablet|descktop]
function mediaCase()
{
...
/*set doctor image as link so that ,by clikcing it,it will go to doctor page */
function activeDoctorImageAsLink()
{
    ...
    $('.doc-card img')
        .css('cursor', 'pointer')
        .click(function () {           
            location.href = defaultPathForDoctorPage + id;
        });
}
...
function onBooking(contextButton)
{
    ...
    //check if user selected appointement
    ...
    if (selectedAppointement.length != 1) {
        ...
        return;//user haden't selected appointement
    }
